...
public class UpdateArticleServlet extends HttpServlet {
	...
	*/
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int updated = 0;
		String resultMessage = SolideConstants.UNSUCCESSFULL_ARTICLE_UPDATED;
		try {
...
//				}
				...
				if (! items.isEmpty()) {
					...
					while (iter.hasNext()) {
						...
						{
							if(item.getFieldName().equals("ArticleNo"))
							{
								articleBean.setArticleNo(Integer.parseInt(item.getString()));
							}
			...
			updated = articleFacade.updateArticle();
			if (updated == -2) {
				resultMessage = SolideConstants.NOT_POSSIBLE_TO_CHANGE_RESERVED_ARTICLE;
			}
			else if (updated == 0) {
				resultMessage = SolideConstants.SUCCESSFULL_ARTICLE_UPDATED;
			} else {
				resultMessage = SolideConstants.UNSUCCESSFULL_ARTICLE_UPDATED;
			}
